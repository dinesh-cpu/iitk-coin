	...
	case "POST":
		...
		for rows.Next() {
			...
			if ID == cred.Id {
				...
				if STAT != "pending" {
					fmt.Fprintf(w, "action is alredy taken")
					return
				}
				if cred.Action == "reject" {
					fmt.Println(ROLL)

					_, err = database.Exec("UPDATE REDEEM SET status = ? WHERE id = ?", "rejected", ID)

					if err != nil {
						fmt.Fprintf(w, "Server error")
						panic(err)
					}
					fmt.Fprintf(w, "action is  taken")
					return
				}
				if cred.Action == "approve" {
					_, err = database.Exec("UPDATE REDEEM SET status = ? WHERE id = ?", "approved", ID)
					if err != nil {
						fmt.Fprintf(w, "Server error")
						panic(err)

					}
					_, err = database.Exec("UPDATE FINALDATA SET coin = coin - ? WHERE rollno = ? AND coin - ? >= 0", COIN, ROLL, COIN)
					if err != nil {
						panic(err)
					}
					...
					addtrans.Exec(ROLL, 190304, COIN, 0, time.Now().String())
					return
